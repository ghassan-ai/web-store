  ...
  return (
    <div className="min-h-screen">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-10">
        {/* Breadcrumb */}
        <nav className="text-sm text-text-dark-muted mb-4 font-tajawal">
          ...
          <span className="mx-2">&gt;</span>
          <span className="text-text-dark-heading">{product.name}</span>
        </nav>
        ...
        <div className="bg-surface-card rounded-2xl border border-surface-border shadow-sm overflow-hidden">
          <div className="grid md:grid-cols-2 gap-0">
            ...
            <div className="p-6 sm:p-8 flex flex-col" dir="rtl">
              <h1 className="text-xl sm:text-2xl font-bold text-text-dark-heading mb-3 font-tajawal">
                {product.name}
              </h1>
              ...
              {product.specs && (
                <div className="mb-4 text-sm text-text-dark-body whitespace-pre-line border-t border-surface-border pt-3 font-tajawal">
                  {product.specs}
                </div>
              )}
              ...
              {product.colors?.length > 0 && (
                <div className="mb-4">
                  <div className="flex items-center gap-3">
                    <p className="text-sm font-semibold text-text-dark-muted font-tajawal">اللون:</p>
                    {selectedColor && (
                      <span className="text-xs text-text-dark-muted/70">{selectedColor}</span>
                    )}
                  </div>
                    ...
                    {product.colors.map((color, i) => (
                      <button
                        ...
                        className={`w-8 h-8 rounded-full transition-all ${
                          selectedColor === color
                            ? 'ring-2 ring-offset-2 ring-accent ring-offset-white scale-110'
                            : 'hover:scale-110'
                        }`}
              ...
              {product.customOptions?.length > 0 && product.customOptions.map((group) => (
                <div key={group.id} className="mb-4">
                  <p className="text-sm font-semibold text-text-dark-muted font-tajawal mb-2">{group.title}:</p>
                      ...
                      <button
                        ...
                        className={`px-4 py-2 rounded-full text-sm font-medium transition-all border ${
                          selectedOptions[group.id] === val
                            ? 'bg-accent text-white border-accent ring-2 ring-offset-2 ring-accent ring-offset-white scale-105'
                            : 'bg-slate-50 border-surface-border text-text-dark-muted hover:border-accent hover:text-text-dark-heading hover:scale-105'
                        }`}
                      >
                        {val}
                      </button>
              ...
              {!isOutOfStock && (
                <div className="mb-6">
                  <p className="text-sm font-semibold text-text-dark-muted mb-2 font-tajawal">الكمية:</p>
                  <div className="flex items-center gap-3">
                    <button
                      onClick={() => setQuantity((q) => Math.max(1, q - 1))}
                      className="w-9 h-9 rounded-lg border border-surface-border flex items-center justify-center text-text-dark-heading hover:bg-slate-50 transition font-bold"
                    >
                      -
                    </button>
                    <span className="text-lg font-bold text-text-dark-heading w-8 text-center font-mono">{quantity}</span>
                    <button
              ...
              <div className="flex flex-col sm:flex-row gap-3 mt-auto">
                <button
                  ...
                  className="flex-1 py-3.5 bg-accent text-white font-bold rounded-xl hover:bg-accent-hover transition-colors disabled:bg-slate-100 disabled:text-text-dark-muted disabled:cursor-not-allowed flex items-center justify-center gap-2 font-tajawal"
                >
        ...
        {similarProducts.length > 0 && (
          <div className="mt-10">
            <h2 className="text-xl sm:text-2xl font-bold text-text-dark-heading mb-6 font-tajawal" dir="rtl">

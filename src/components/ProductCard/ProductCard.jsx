...
const ProductCard = React.memo(function ProductCard({ product, viewMode = "grid" }) {
  ...
  if (viewMode === "list") {
    return (
      <div
        ...
        className="flex items-center gap-4 bg-card rounded-xl p-3 border border-card-border hover:border-accent/40 transition-all cursor-pointer"
      >
        ...
        <div className="flex-1 min-w-0">
          <h3 className="text-sm font-semibold text-white truncate font-tajawal">{product.name}</h3>
        </div>
        <div className="flex flex-col items-end gap-2 flex-shrink-0">
          ...
          <div className="flex gap-1">
            <button
              ...
              className="w-8 h-8 rounded-lg bg-accent text-white flex items-center justify-center hover:bg-accent-hover transition-all disabled:bg-card-border disabled:text-text-secondary disabled:cursor-not-allowed"
            >
  ...
  return (
    <div
      onClick={() => router.push(`/product/${product.id}`)}
      className="group rounded-2xl overflow-hidden bg-card border border-card-border hover:border-accent/40 hover:-translate-y-1 transition-all duration-400 cursor-pointer"
    >
      {/* Image */}
      ...
      <div className="p-4 sm:p-5">
        <h3 className="text-sm font-semibold text-white line-clamp-2 mb-2.5 min-h-[2.5rem] font-tajawal">
          {product.name}
        </h3>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button
              onClick={(e) => { e.stopPropagation(); addToCart(product); }}
              disabled={isOutOfStock}
              className="h-9 px-3.5 rounded-xl bg-accent text-white text-xs font-medium flex items-center gap-1.5 hover:bg-accent-hover transition-all duration-200 disabled:bg-card-border disabled:text-text-secondary disabled:cursor-not-allowed"
            >
              <ShoppingCart size={14} />

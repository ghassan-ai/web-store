  ...
  return (
    <div className="rounded-2xl overflow-hidden animate-pulse bg-card border border-card-border">
      ...
      <div className="p-4 space-y-2">
        <div className="h-4 bg-primary/50 rounded w-2/3" />
        <div className="h-5 bg-primary/50 rounded w-1/4" />
      </div>
    </div>
  );
}
...
const ArrivalCard = React.memo(function ArrivalCard({ product }) {
  ...
  return (
    <Link href={`/product/${product.id}`} className="group rounded-2xl overflow-hidden bg-card border border-card-border hover:border-accent/40 hover:-translate-y-1 transition-all duration-400">
      ...
      <div className="p-4 sm:p-5">
        <h3 className="text-sm font-semibold text-white line-clamp-2 mb-3 font-tajawal">{product.name}</h3>
